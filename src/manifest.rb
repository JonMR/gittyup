Dir.glob(File.expand_path(File.dirname(__FILE__) + "/**/*").gsub('%20', ' ')).each do |directory|
	next if directory =~ /\/diff-lcs-[0-9.]+\// or directory =~ /\/grit-[0-9.]+\// or directory =~ /\/mime-types-[0-9.]+\//
...
when Monkeybars::Resolver::IN_FILE_SYSTEM
  add_to_classpath '../lib/java/monkeybars-1.1.1.jar'
end
...
when Monkeybars::Resolver::IN_FILE_SYSTEM
  ...
  add_to_load_path "../lib/ruby/diff-lcs-1.1.3/lib"
  add_to_load_path "../lib/ruby/grit-2.4.1/lib"
  add_to_load_path "../lib/ruby/mime-types-1.17.2/lib"
